...
/*LEETCODE:1926:NEAREST EXIT FROM ENTRANCE FROM MAZE: 
 * 
    ...
    public int nearestExit(char[][] maze, int[] entrance) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] dir : directions) {
                ...
                // Check if the new position is within the bounds and is an open path
                if (newRow >= 0 && newRow < rows && newCol >= 0 && newCol < cols && maze[newRow][newCol] == '.') {
                    // Check if it's an exit (boundary cell but not the entrance)
